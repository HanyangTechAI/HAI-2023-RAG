...
emb_func = SentenceTransformerEmbeddingFunction(
    model_name=os.environ.get("EMBEDDING_MODEL"), device="cuda"
)
...
def create_collection(collection_name: str, metadata: dict = None):
    ...
    return client.get_or_create_collection(
        name=collection_name, embedding_function=emb_func, metadata=metadata
    )
...
def index_documents(collection_name: str, document: dict):
    ...
    collection = client.get_collection(collection_name, embedding_function=emb_func)
