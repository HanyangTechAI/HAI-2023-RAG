...
with col0.form("Settings", clear_on_submit=False):
    ...
    collection_name = st.text_input(label = "Enter the name of database to use.",
                                key = "collection_name")
    st.text_area("Input system messages below:", key="system_message", label_visibility="visible")
...
with col1.form("Upload File", clear_on_submit=False):
    ...
    st.markdown("- Once the file is entered, it is automatically split into pages and stored in the DB.\n- When you type a question, it will answer based on the most relevant pages.")
    uploaded_file = st.file_uploader("Uplode files",
